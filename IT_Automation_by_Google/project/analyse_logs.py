"""Script to analyse logs and create CSV files with the results"""
...
csv_file_location = os.path.join(data_directory, "syslog.log")
with open(csv_file_location, encoding="UTF-8") as file:
    # Read each line
...
data_directory = os.path.dirname(os.path.abspath(__file__))
user_file_location = os.path.join(data_directory, "user_statistics.csv")
with open(user_file_location, "w", newline="", encoding="UTF-8") as user_csv:
    for key, value in per_user_list:
        user_csv.write(
            str(key) + "," + str(value["INFO"]) + "," + str(value["ERROR"]) + "\n"
        )

# * Create CSV error_message
data_directory = os.path.dirname(os.path.abspath(__file__))
error_file_location = os.path.join(data_directory, "error_statistics.csv")
with open(error_file_location, "w", newline="", encoding="UTF-8") as error_csv:
